...
fp3 = open("output_00.txt",'w')
fp3.close()

#this is the command this helps to choose which file to run
command = "ngspice destination_first_00.ckt"
...
for j in range(0,8): #j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    ...
    for i in range(0,4):  #i is for output sequence
        ...
        if(j<4):
            fp1 = open("first_A_add.ckt",'r') #for input sequence 1
        else:
            fp1 = open("first_B_add.ckt",'r') #for input sequecne 2
        #if you have any other input sequences you can add

        fp2 =open("destination_first_00.ckt",'w') #destination file for running
        fp3 = open("output_00.txt",'a') #final output file
        mode1 = "RISE"
        mode2 = "RISE"
        mode3 = "FALL"
        mode4 = "FALL"
