...
fp3 = open("output_10.txt",'w')
fp3.close()

#this is the command this helps to choose which file to run
command = "ngspice destination_first_10.ckt"

#start of script
for j in range(0,8): #j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    if j<4:
        s1 = "INPUT_ALU_A"+str(j)   #for input sequence 0000 1111 output = 1111 s1 indicates string1
    else:
        s1 = "INPUT_ALU_B"+str(j-4) #for input sequence 1111 0000 output = 1111

      #i is for output sequence
    
    for i in range(0,3):
        if i == 0:
            s2 = "COMPARATOR_OUT_A_GREATERTHAN_B"
        elif i == 1:
            s2 = "COMPARATOR_A_EQUAL_B_FINAL"
        elif i == 2:
            s2 = "COMPARATOR_OUT_A_LESSTHAN_B"
        #fp1 is src file since you cant modify it you open it in read mode
        
        if(j<4):
            fp1 = open("first_A_comp.ckt",'r') #for input sequence 1
        else:
            fp1 = open("first_B_comp.ckt",'r') #for input sequecne 2
        #if you have any other input sequences you can add

        fp2 =open("destination_first_10.ckt",'w') #destination file for running
        fp3 = open("output_10.txt",'a') #final output file
